...
기능:
  1. 기본 인적 정보 입력
  2. 사용자 입력문장 감정 추출 (챗봇)
  3. 사용자 입력문장 NLP 감정 분석 리포트 (전용 페이지)
  4. 사용자 우울 자가진단 (PHQ-9)
  5. 우울 위험도 3단계 맞춤 케어
  6. 공공 서비스 연계
  7. 상담사 대시보드
...
MIN_WORDS_PER_TURN      = 3    # 발화당 최소 유효 단어 (의미있는 발화 필요)
...
PHQ9_SEVERITY = [
    (0,4,"없음-최소","#E8F5EF","#2A6049","추가 조치 불필요"),
    (5,9,"경증","#FEFAE8","#6B5A20","추적 관찰 권장"),
    (10,14,"보통","#FFF0D8","#8B5A20","상담·치료 계획 고려"),
    (15,19,"중등도","#FFE0D0","#8B3A20","전문가 상담 권고"),
    (20,27,"극심한 우울","#FFD8D8","#8B2020","정신건강 전문가 의뢰"),
]
COMMUNITY_PROGRAMS = [
...
def kiwi_tokenize(text: str) -> list:
    ...
    for t in _kiwi_instance.tokenize(text, stopwords=None):
        ...
        if form in _NEGATION_FORMS or form in _EMPHASIS_FORMS:
            result.append(form)
        elif tag in _KEEP_TAGS and len(form) >= 2:
            result.append(form)
    return result


def count_valid_words_kiwi(text: str) -> int:
    """한 발화의 유효 단어 수 반환 (Jin et al. 2023: 최소 3단어 기준)."""
    return len(kiwi_tokenize(text))
...
def make_wordcloud_png(texts:list):
    """PNG bytes 반환 + 유효 단어 수.
    kiwi_tokenize 직접 적용 → 부정어("안","못","없다","아니") + 강조어("너무","정말","매우","엄청") 보존.
    각 발화를 개별 토크나이징 후 합산 (joined 방식보다 형태소 경계 보존이 정확함).
    """
    if not _WORDCLOUD_OK or not texts: return None, 0
    all_tokens = []
    for t in texts:
        t_clean = re.sub(r"[^\w가-힣\s]", " ", t)
        all_tokens.extend(kiwi_tokenize(t_clean))
    ...
    try:
        wc = WordCloud(
            ...
            prefer_horizontal=0.9, relative_scaling=0.4, max_words=50,
        ).generate_from_frequencies(freq)
    except: return None, unique_count
    fig, ax = plt.subplots(figsize=(7,4.2), dpi=120)
    ax.imshow(wc, interpolation="bilinear"); ax.axis("off"); fig.tight_layout(pad=0)
    buf = io.BytesIO()
...
if st.session_state.page == "💬 일상 대화":
    ...
    with col_chat:
        if not st.session_state.profile_done:
            ...
            with st.form("profile_form"):
                ...
                with ca: age_group  = st.selectbox("연령대", ["10대","20대","30대","40대","50대 이상"])
            ...
            if submit and user_input.strip():
                ...
                st.session_state.risk_history.append({"turn":len(st.session_state.risk_history)+1,"level":analysis["level"],"score":analysis["score"],"text":user_input.strip()[:60],"top_emos":top_emos})
                st.rerun()
            ...
            if n_cur > 0:
                ...
                if n_cur >= MIN_TURNS_FOR_ANALYSIS:
                    if st.button("📊 대화 감정 분석 보기 →", key="go_to_analysis", type="primary"):
                        st.session_state.page="📊 감정 분석"; st.rerun()
                else:
                    remain = MIN_TURNS_FOR_ANALYSIS - n_cur
                    st.info(f"📊 감정 분석은 최소 {MIN_TURNS_FOR_ANALYSIS}회 이상 대화했을 때 가능해요. (현재 {n_cur}회 — {remain}회 더 필요)")
    ...
    with col_right:
        ...
        if cond:
            st.markdown('<div class="cond-met">✅ 충족 — 감정 분석 페이지에서 NLP 리포트 확인 가능</div>', unsafe_allow_html=True)
        else:
            remain = MIN_TURNS_FOR_ANALYSIS - n_cur
            st.markdown(f'<div class="cond-unmet">💬 {remain}회 더 대화하면 NLP 분석이 열립니다</div>', unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown('<div class="sec-header">🆘 위기 상담</div>', unsafe_allow_html=True)
        for num,name,desc in [("1393","자살예방상담전화","24시간"),("1577-0199","정신건강위기상담","24시간"),("129","보건복지상담","복지 연계"),("1388","청소년상담전화","24시간")]:
...
# ═══════════════════════════════════════════════════════════════
    ...
    if n_turns == 0:
        ...
        with dc1:
            ...
            # 3단어 이상 유효 발화만 추출 (Chen 2021 + Jin et al. 2023 기준)
            valid_msgs   = [m for m in user_msgs if count_valid_words_kiwi(m) >= MIN_WORDS_PER_TURN]
            ...
            if not _WORDCLOUD_OK:
                ...
                st.info(
                    f"📝 워드클라우드는 의미 있는 발화가 조금 더 쌓이면 보여드려요.\n\n"
                    f"**워드클라우드 표시 조건**\n"
                    f"- ✅ 대화 횟수: {n_user_turns}회 / 최소 {MIN_TURNS_FOR_WORDCLOUD}회\n"
                    f"- {'✅' if n_valid >= MIN_TURNS_FOR_WORDCLOUD else '⏳'} 3단어 이상 의미 있는 발화: {n_valid}회 / 최소 {MIN_TURNS_FOR_WORDCLOUD}회 (앞으로 {remain_valid}회 더 필요)\n\n"
                    f"_💡 3단어 이상으로 이야기해 주시면 더 정확한 워드클라우드를 만들 수 있어요!_"
                )
            else:
                wc_bytes, uniq_count = make_wordcloud_png(valid_msgs)
                if wc_bytes and uniq_count >= MIN_CUMULATIVE_WORDS:
                    st.image(wc_bytes, use_container_width=True,
                             caption=f"대화에서 자주 언급된 단어 (유효 단어 {uniq_count}개 · 부정어·강조어 보존)")
                elif wc_bytes:
                    st.info(f"📝 다양한 단어가 좀 더 필요해요. (현재 고유 단어 {uniq_count}개 / 필요 {MIN_CUMULATIVE_WORDS}개 이상)")
                else:
                    st.info("아직 워드클라우드를 생성할 만큼 충분한 텍스트가 모이지 않았어요.")
        ...
        st.markdown("---")
        if st.button("🌱 맞춤 케어 보기 →", key="goto_care_nlp", type="primary"):
            st.session_state.page="🌱 맞춤 케어"; st.rerun()
...
# ═══════════════════════════════════════════════════════════════
    ...
    if not a:
        ...
        if st.button("💬 일상 대화 시작하기", key="goto_chat_2", type="primary"): st.session_state.page="💬 일상 대화"; st.rerun()
    else:
        ...
        if not phq9_done:
            st.markdown('<div style="background:#EDF2FF;border-radius:14px;padding:1.4rem 1.7rem;border:1px solid #BFD0EE;text-align:center;"><div style="font-size:1.4rem;margin-bottom:0.4rem;">📝</div><div style="font-weight:700;font-size:0.97rem;color:#3A5A9A;margin-bottom:0.4rem;">PHQ-9 우울 자가검진을 먼저 진행해 주세요</div><div style="font-size:0.86rem;color:#5A7ABE;line-height:1.7;">맞춤 케어는 PHQ-9 결과를 기반으로 제공됩니다.</div></div>', unsafe_allow_html=True)
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("📝 PHQ-9 검진 하러 가기", type="primary", use_container_width=True): st.session_state.page="📝 PHQ-9 검진"; st.rerun()
        elif cl == "양호":
            ...
            with cc2:
                ...
                for nu,nm,de in [("1393","자살예방상담전화","24시간"),("1577-0199","정신건강위기상담","24시간"),("129","보건복지상담","복지 연계"),("1388","청소년상담전화","24시간")]:
                ...
                lifecycle={"10대":"학교 상담실, 청소년상담전화(1388), 또래 멘토링","20대":"청년 불씨 프로젝트, 대학 상담센터, 마인드부스터 Green","30대":"직장인 EAP, 지역 정신건강복지센터","40대":"중년 심리지원 프로그램","50대 이상":"노인 돌봄 서비스, 복지관 프로그램"}
        ...
        st.markdown("---")
        if st.button("📝 PHQ-9 우울 자가검진 →", type="primary", key="btn_care_to_phq"): st.session_state.page="📝 PHQ-9 검진"; st.rerun()
...
# ═══════════════════════════════════════════════════════════════
    ...
    if st.session_state.phq9_done:
        ...
        st.markdown('<div style="font-size:0.76rem;color:#A0AEC0;text-align:center;margin-top:-0.4rem;"><span style="color:#5B9E7A;">■</span> 없음(0~4) · <span style="color:#C4A830;">■</span> 경증(5~9) · <span style="color:#D08840;">■</span> 보통(10~14) · <span style="color:#C06848;">■</span> 중등도(15~19) · <span style="color:#B04040;">■</span> 극심(20~27)</div>', unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════
    ...
    with sc1:
        st.markdown('<div class="sec-header">🆘 위기 상담 핫라인</div>', unsafe_allow_html=True)
        for nu,nm,de in [("1393","자살예방상담전화","24시간 무료"),("1577-0199","정신건강위기상담전화","24시간"),("129","보건복지상담센터","복지 연계"),("1388","청소년상담전화","24시간")]:
    ...
    with sc2:
        ...
        else: st.info("💡 PHQ-9 검진을 완료하면 맞춤 서비스를 추천해 드립니다.")
    st.markdown("---")
    pb1,pb2=st.columns(2)
    with pb1:
        if st.button("📝 PHQ-9 검진 →",key="goto_phq_1",type="primary",use_container_width=True): st.session_state.page="📝 PHQ-9 검진"; st.rerun()
    with pb2:
...
# ═══════════════════════════════════════════════════════════════
    ...
    st.markdown('<div style="background:linear-gradient(135deg,#5B82B5 0%,#7B9FD0 55%,#8B7BAD 100%);border-radius:14px;padding:0.9rem 1.4rem;color:white;margin-bottom:1.1rem;"><div style="font-size:1.05rem;font-weight:900;margin-bottom:0.2rem;">🩺 상담사 전용 대시보드</div><div style="font-size:0.82rem;opacity:0.92;">대화 단위 위험도 · 표현 패턴 · 우선 개입 대상</div></div>', unsafe_allow_html=True)
    ...
    if not rh:
        ...
        tt=len(rh); ht=sum(1 for r in rh if r["score"]>=60 or any(e in HIGH_RISK for e in r.get("top_emos",[])))
        mt=sum(1 for r in rh if 35<=r["score"]<60); av=sum(r["score"] for r in rh)/tt
        sm1,sm2,sm3,sm4=st.columns(4)
        sm1.metric("총 대화 턴",f"{tt}회"); sm2.metric("🔴 고위험 턴",f"{ht}회",f"{ht/tt*100:.0f}%")
        sm3.metric("🟠 중등도 턴",f"{mt}회"); sm4.metric("평균 위험도",f"{av:.1f}점")
        if ht>0: st.markdown(f'<div style="background:#FDF6F6;border:1.5px solid {P_WARM};border-radius:12px;padding:0.9rem 1.3rem;margin:0.7rem 0;font-size:0.88rem;color:#6A3030;line-height:1.8;">⚠️ <b>우선 개입 대상</b> — 고위험 발화 <b>{ht}회</b> 감지<br><span style="font-size:0.81rem;">☎ 1393 / 1577-0199</span></div>', unsafe_allow_html=True)
        elif mt>=2: st.markdown(f'<div style="background:#EDF2FF;border-radius:10px;padding:0.7rem 0.9rem;font-size:0.85rem;color:#3A5A9A;border-left:4px solid {P_PURPLE};">⚠️ 중증 위험 표현 {mt}회 감지. 전문 상담 연계를 권고합니다.</div>', unsafe_allow_html=True)
        dc1,dc2=st.columns([1.4,1],gap="large")
        with dc1:
            st.markdown('<div class="sec-header">📈 위험도 타임라인</div>', unsafe_allow_html=True)
            def s2c(s):
                if s>=60: return P_WARM
                elif s>=35: return P_PINK
                elif s>=15: return "#C4A857"
                return P_ROSE
            fig_tl=go.Figure()
            fig_tl.add_trace(go.Scatter(x=[r["turn"] for r in rh],y=[r["score"] for r in rh],mode="lines+markers",line=dict(color="#8B7BAD",width=2),marker=dict(color=[s2c(r["score"]) for r in rh],size=10,line=dict(color="white",width=1.5)),hovertemplate="턴 %{x}<br>위험도: %{y:.1f}<extra></extra>"))
            fig_tl.update_layout(height=200,margin=dict(l=0,r=0,t=8,b=28),paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",xaxis=dict(title="대화 턴",gridcolor="#EEF4FA",dtick=1),yaxis=dict(title="위험도",gridcolor="#EEF4FA"))
            st.plotly_chart(fig_tl,use_container_width=True)
            st.markdown('<div class="sec-header">📋 턴별 위험도 상세</div>', unsafe_allow_html=True)
            for r in rh:
                sc=r["score"]
                if sc>=60: badge='<span class="risk-badge-high">🔴 고위험</span>'
                elif sc>=35: badge='<span class="risk-badge-mid">🟠 중등도</span>'
                elif sc>=15: badge='<span class="risk-badge-low">🟡 경도</span>'
                else: badge='<span class="risk-badge-safe">🟢 양호</span>'
                st.markdown(f'<div style="background:#F5F8FC;border-radius:9px;padding:0.55rem 0.9rem;margin-bottom:0.3rem;font-size:0.84rem;display:flex;align-items:center;gap:0.5rem;"><span style="font-weight:700;color:{P_PEACH};min-width:36px;">#{r["turn"]}</span>{badge}<span style="color:#5A7A96;flex:1;">{r["text"]}{"..." if len(r["text"])>=60 else ""}</span></div>', unsafe_allow_html=True)
        with dc2:
            st.markdown('<div class="sec-header">🔍 감정 발생 패턴</div>', unsafe_allow_html=True)
            ae={}
            for r in rh:
                for emo,prob in r["top_emos"]: ae[emo]=ae.get(emo,0)+prob
            if ae:
                for emo,tot in sorted(ae.items(),key=lambda x:x[1],reverse=True)[:8]:
                    ir=emo in HIGH_RISK; c=P_WARM if ir else P_CORAL
                    bw=max(int(min(tot/tt*100,100)*1.8),4)
                    tag=f'<span style="background:#F5E5E5;color:#6A3030;border-radius:5px;padding:0.12rem 0.45rem;font-size:0.76rem;font-weight:600;">⚠ 위험</span>' if ir else ""
                    st.markdown(f'<div style="background:white;border-radius:8px;padding:0.48rem 0.75rem;margin-bottom:0.3rem;border-left:3px solid {c};"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.18rem;"><span style="font-weight:600;font-size:0.84rem;color:{c};">{emo}</span>{tag}<span style="font-size:0.76rem;color:#A0AEC0;">{tot*100:.0f}% 누적</span></div><div style="background:#EEF4FA;border-radius:4px;height:4px;"><div style="background:{c};height:4px;border-radius:4px;width:{bw}%;max-width:100%;"></div></div></div>', unsafe_allow_html=True)

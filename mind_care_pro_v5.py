...
# [요청 9] 워드클라우드 관련 import (선택 의존성 — 없으면 기능만 비활성화)
try:
    import io
    import re
    ...
    _WORDCLOUD_OK = False
    _WORDCLOUD_ERR = str(_e)

# [요청 7] 감정 분석을 위해 필요한 최소 대화 턴 수
MIN_TURNS_FOR_ANALYSIS = 3

# [워드클라우드 노출 조건]
# 의미 있는 워드클라우드를 만들려면 일정 분량 이상의 텍스트가 필요함
MIN_TURNS_FOR_WORDCLOUD = 3     # 최소 대화 턴 수
MIN_CHARS_FOR_WORDCLOUD = 50    # 전체 사용자 발화의 총 글자 수
MIN_WORDS_FOR_WORDCLOUD = 10    # 불용어 제거 후 유효 단어 개수

# [요청 8] 차트 단일 색상 통일
CHART_PRIMARY_COLOR = "#2563EB"  # 감정별 평균 확률·PHQ-9 문항별 응답 공통 색
...
# ─────────────────────────────────────────────────
# ─────────────────────────────────────────────────
# 공통 페르소나 지침 — 19개 감정 라벨 유도 + 감정 표현 촉진
# ─────────────────────────────────────────────────
# 모든 페르소나에 공통으로 주입되는 "감정 유도 가이드"
# (사용자 문장이 짧을 때도 감정이 잘 드러나도록 후속 질문·감정 표현 촉진)
COMMON_PERSONA_GUIDE = """
[공통 응답 규칙 — 반드시 지켜야 함]
- 감정 분석 모델은 19가지 감정 라벨(불안, 분노, 슬픔, 상처, 당황, 기쁨, 감사, 사랑, 외로움, 우울감, 무기력, 절망감, 죄책감, 자살충동, 스트레스, 자존감 저하, 대인관계, 일상, 기타)을 감지합니다.
- 사용자가 자신의 감정을 구체적으로 표현할 수 있도록 다음을 유도하세요:
  1) 사용자의 말이 짧거나 단답이면 반드시 "지금 기분을 한 단어로 표현한다면?", "몸에서는 어떤 느낌이 드세요?", "최근에 가장 인상 깊었던 순간은?" 같은 감정·상황 탐색 질문을 하나 덧붙이세요.
  2) 사용자의 발화에서 감정 단어를 포착하면 그 감정을 반영하는 공감 문장을 먼저 말하고, 그 뒤에 열린 질문을 이어가세요.
  3) 우울·무기력·자살충동·절망감 같은 위험 신호가 보이면 가볍게 넘기지 말고 반드시 진지하게 받아들이세요. 바로 전화번호부터 들이밀지 말고 공감 → 상황 확인 → 도움 리소스 안내 순서를 지키세요.
  4) 일상·기쁨·감사 같은 긍정 감정도 놓치지 말고 "그 순간 어떤 기분이 들었어요?"처럼 되짚어 주세요.
- 답변은 자연스러운 문장으로만 작성하세요. 목록·불릿·이모지 나열은 피하세요.
"""
...
- 예: "그런 마음이 드셨군요. 오늘 하루가 특히 힘드셨나 봐요. 그때 어떤 생각이 드셨는지 조금 더 들려주실 수 있을까요?"
""" + COMMON_PERSONA_GUIDE,
        "color": "#4A90D9",
    },
    "👫 친구 민준": {
        "description": "편하게 털어놓을 수 있는 동네 친구",
        "system": """너는 '민준'이라는 이름의 오래된 친구야. 반말로 대화해.

[역할]
- 상담사처럼 딱딱하게 말하지 말고, 진짜 친구가 옆에서 들어주듯 편하게 반응해.
- 판단하지 않고 "야 그랬구나", "진짜 힘들었겠다" 같은 동네 친구 말투를 써.
- 격식체·존댓말 금지. '~요' 끝맺음 금지.

[말투·길이]
- 한 번 응답할 때 80자~150자 사이로 짧고 친근하게 작성해.
- 구조: 공감 한 마디 → 친구로서 상황 되묻기 한 문장. 길게 설교 금지.
- 예: "아 진짜? 그거 듣기만 해도 답답하네 ㅜㅜ 너는 그때 기분이 어땠어?"
""" + COMMON_PERSONA_GUIDE,
        "color": "#27AE60",
    },
    "🤖 AI 어시스턴트 클로": {
...
- 구조: 감정 요약 → 사실 확인 질문 → (필요 시) 간단한 자원 제안.
""" + COMMON_PERSONA_GUIDE,
        "color": "#8E44AD",
    },
    "📚 멘토 선생님": {
...
[말투·길이]
- 한 번 응답할 때 150자~250자 사이로 작성합니다.
- 구조: 공감 → "이런 관점은 어떠세요?" 식 생각거리 → 열린 질문.
""" + COMMON_PERSONA_GUIDE,
        "color": "#E67E22",
    },
    "😄 개그맨 철수": {
...
[역할]
- 가볍고 긍정적인 에너지로 대화해. 하지만 상대 감정을 무시하지 마.
- 상대가 진짜 힘들어 보이면 유머는 잠시 내려놓고 진심으로 공감해.

[말투·길이]
- 한 번 응답할 때 80자~150자 사이로 짧고 경쾌하게.
- 예: '그건 진짜 웃픈 일이네 ㅋㅋ', '야 그거 레전드다', '아 진짜? ㄷㄷ'
- 이모지 사용 OK (😂🤣😆) — 진지한 순간엔 자제.
""" + COMMON_PERSONA_GUIDE,
        "color": "#E74C3C",
    },
}
...
def get_avg_probs(exclude_daily: bool = False, min_pct: float = 0.0):
    """전체 대화의 감정별 평균 확률 계산.

    Parameters
    ----------
    exclude_daily : bool
        True면 '일상'(index 11)을 제외 — 위험 감정 집계용.
        False면 '일상' 포함 — 대화 감정 분석 기본 표시용(요청 2).
    min_pct : float
        이 값(%) 미만인 감정은 결과에서 제외. 예: 1.0 → 1% 미만 필터링(요청 6).
    """
    ...
    for i, name in INV_MAP.items():
        if exclude_daily and i == 11:
            continue
        pct = round(float(avg[i]) * 100, 1)
        if pct < min_pct:
            continue
        result[name] = pct
    return result
...
# ─────────────────────────────────────────────────
# [요청 10] 프로필·감정 정보를 시스템 프롬프트에 동적으로 주입
# ─────────────────────────────────────────────────
def build_dynamic_system(base_system: str, profile: dict, analysis) -> str:
    """페르소나 기본 시스템 프롬프트에 사용자 프로필·최근 감정을 덧붙여
    챗봇이 연령·성별·관심사·감정 상태를 반영해 응답하도록 만든다.
    """
    parts = [base_system]
    ...
    if profile:
        ...
        age_group  = profile.get("age_group", "").strip()
        gender     = profile.get("gender", "").strip()
        occupation = profile.get("occupation", "").strip()
        region     = profile.get("region", "").strip()
        interests  = profile.get("interests", []) or []

        persona_ctx = "\n[사용자 정보 — 응답 시 자연스럽게 반영할 것]\n"
        if nickname:   persona_ctx += f"- 호칭: {nickname}님 (반말 페르소나면 '{nickname}아/야')\n"
        if age_group:  persona_ctx += f"- 연령대: {age_group} — 이 세대가 공감할 예시·표현을 사용하세요.\n"
        if gender:     persona_ctx += f"- 성별: {gender}\n"
        if occupation: persona_ctx += f"- 직업: {occupation} — 직업 맥락의 스트레스 요인을 헤아리세요.\n"
        if region:     persona_ctx += f"- 거주 지역: {region}\n"
        if interests:  persona_ctx += f"- 관심사: {', '.join(interests)} — 필요 시 관심사를 가볍게 언급해 친밀감을 높이세요.\n"
        parts.append(persona_ctx)

    # 2) 최근 감지된 감정 상태 (직전 분석 결과)
    if analysis:
        level   = analysis.get("level", "")
        score   = analysis.get("score", 0)
        probs   = analysis.get("probs", None)
        if probs is not None:
            # 상위 3개 감정 추출 (일상 제외)
            top = sorted(
                [(INV_MAP[i], float(probs[i])) for i in range(len(probs)) if i != 11],
                key=lambda x: x[1], reverse=True
            )[:3]
            top_str = ", ".join([f"{n}({v*100:.0f}%)" for n, v in top])
            emo_ctx = (
                "\n[직전 발화의 감정 분석 결과 — 응답 톤 조절용]\n"
                f"- 위험도: {level} (점수 {score:.1f}/100)\n"
                f"- 상위 감정: {top_str}\n"
                "- 위 감정 중 우울감·무기력·절망감·자살충동 같은 위험 신호가 있으면 더 신중·진지하게 공감하세요.\n"
                "- 기쁨·감사 같은 긍정 감정이 상위면 그 순간을 축하·확장해 주세요.\n"
            )
...
def make_wordcloud_png(texts: list[str]) -> tuple[bytes | None, int]:
    """사용자 발화 리스트로 워드클라우드 PNG 바이트를 생성.

    Returns
    -------
    (png_bytes 또는 None, 유효 단어 개수)
    유효 단어 개수는 불용어를 제외하고 남은 단어의 개수.
    호출부에서 이 개수를 보고 워드클라우드를 실제로 노출할지 결정 가능.
    """
    if not _WORDCLOUD_OK or not texts:
        return None, 0

    # 텍스트 정제: 한글·영문·숫자만 남기고 나머지 제거
    joined = " ".join(texts)
    joined = re.sub(r"[^\w가-힣\s]", " ", joined)
    tokens = [t for t in joined.split() if len(t) >= 2 and t not in _KO_STOPWORDS]
    ...
    font_path = _find_ko_font()
    try:
        wc = WordCloud(
            font_path=font_path,
            width=600, height=400,
            background_color="white",
            colormap="Blues",  # 차트 색 통일 톤과 맞춤
        ...
        if not st.session_state.profile_done:
            ...
            if submit and user_input.strip():
                ...
                with st.spinner("답변 생성 중..."):
                    try:
                        # [요청 10] 프로필 + 직전 감정 분석을 시스템 프롬프트에 주입
                        dyn_system = build_dynamic_system(
            ...
            # [요청 7] 감정 분석은 최소 턴 수 이상일 때만 이동 가능
            if st.session_state.probs_history:
                ...
                if n_cur >= MIN_TURNS_FOR_ANALYSIS:
                    ...
                    st.info(f"📊 감정 분석은 최소 {MIN_TURNS_FOR_ANALYSIS}회 이상 대화했을 때 가능해요. "
                            f"(현재 {n_cur}회 — {remain}회 더 필요)")
...
# ═══════════════════════════════════════════════════════════════
    ...
    if n_turns == 0:
        ...
        st.markdown(f"""
        <div style="background:#FEF3C7;border-radius:14px;padding:1.5rem 1.8rem;
                    border:1px solid #FDE68A;text-align:center;">
            <div style="font-size:1.6rem;margin-bottom:0.5rem;">⏳</div>
            <div style="font-weight:700;font-size:1rem;color:#92400E;margin-bottom:0.5rem;">
            ...
            <div style="font-size:0.9rem;color:#78350F;line-height:1.7;">
                ...
                현재 <b>{n_turns}회</b> 진행됨 — <b>{need}회 더</b> 대화를 나눠 주세요!<br>
                <span style="font-size:0.82rem;color:#A16207;">
                짧은 한 문장보다는 그날의 기분·상황을 구체적으로 들려주시면 더 정확해집니다.
                </span>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style="background:#EFF6FF;border-radius:10px;padding:0.7rem 1rem;margin-bottom:1rem;
            ...
            💡 아래 결과는 전체 대화의 <b>감정별 평균 확률</b>입니다.
            일상을 포함한 <b>전체 20개 감정</b>을 확률이 낮아도 모두 표시합니다.
        </div>
        """, unsafe_allow_html=True)

        # [요청 수정] 일상 포함한 20개 감정 모두 표시 (확률이 0에 가까워도 노출)
        avg_probs = get_avg_probs(exclude_daily=False, min_pct=0.0)
        # 위험 감정 분석용으로는 일상 제외 + 필터 없음 버전도 계산
        avg_probs_no_daily = get_avg_probs(exclude_daily=True, min_pct=0.0) or {}

        dc1, dc2 = st.columns([1.5, 1], gap="large")

        with dc1:
            # ── 20개 감정 평균 확률 바차트 (일상 포함, 0%도 표시) ──
            ...
            if not avg_probs:
                ...
                # [요청 8] 색깔 하나로 통일 (단일 파란색)
                # x축 상한은 최대값보다 살짝 여유 있게 — 0에 가까운 값도 보이도록
                ...
                fig_bar.update_layout(
                    ...
                    xaxis=dict(gridcolor="#F1F5F9", title="평균 확률 (%)", range=[0, x_max]),
            ...
            # [조건 체크] 최소 기준 미달이면 워드클라우드 생성 안 함
            n_user_turns = len(user_msgs)
            total_chars  = sum(len(m) for m in user_msgs)

            if not _WORDCLOUD_OK:
                st.warning("워드클라우드 생성에 필요한 패키지가 설치되지 않았습니다. "
                           "터미널에서 `pip install wordcloud matplotlib` 을 실행해 주세요.")
            elif n_user_turns < MIN_TURNS_FOR_WORDCLOUD or total_chars < MIN_CHARS_FOR_WORDCLOUD:
                # 턴 수 또는 글자 수 미달
                st.info(
                    f"📝 워드클라우드는 대화가 조금 더 쌓이면 보여드려요.\n"
                    f"- 현재 대화 {n_user_turns}회 / 필요 {MIN_TURNS_FOR_WORDCLOUD}회 이상\n"
                    f"- 현재 총 {total_chars}자 / 필요 {MIN_CHARS_FOR_WORDCLOUD}자 이상"
                )
            else:
                wc_bytes, uniq_count = make_wordcloud_png(user_msgs)
                if wc_bytes and uniq_count >= MIN_WORDS_FOR_WORDCLOUD:
                    st.image(wc_bytes, use_container_width=True,
                             caption=f"대화에서 자주 언급된 단어 (유효 단어 {uniq_count}개 · 불용어 제외)")
                elif wc_bytes:
                    # 단어는 있지만 고유 단어 수가 기준 미달
                    st.info(
                        f"📝 의미 있는 워드클라우드를 위해 다양한 단어가 좀 더 필요해요. "
                        f"(현재 고유 단어 {uniq_count}개 / 필요 {MIN_WORDS_FOR_WORDCLOUD}개 이상)"
                    )
                else:
            ...
            for i, content in enumerate(user_msgs):
                ...
                <div style="background:#F8FAFC;border-radius:8px;padding:0.5rem 0.9rem;
                            margin-bottom:0.3rem;font-size:0.84rem;border-left:3px solid {CHART_PRIMARY_COLOR};">
        ...
        with dc2:
            ...
            if avg_probs:
                ...
                for rank, (emo, val) in enumerate(sorted_emotions[:3], 1):
                    color = "#DC2626" if emo in HIGH_RISK else CHART_PRIMARY_COLOR
                    medal = ["🥇", "🥈", "🥉"][rank-1]
                    st.markdown(f"""
...
# ═══════════════════════════════════════════════════════════════
    ...
    # [요청 5 수정] "결과 확인하기" 버튼은 상태만 저장. 결과 렌더링은 아래 phq9_done 블록에서 수행.
    ...
    # ─────────────────────────────────────────────────────────
    # [요청 5 수정] 결과 표시 — phq9_done 기반, 버튼 클릭에 의존하지 않음
    # (내부의 "공공 서비스" 버튼을 눌러도 결과가 사라지지 않음)
    # ─────────────────────────────────────────────────────────
    if st.session_state.phq9_done:
        ...
        # [요청 8] 문항별 응답 바차트 색상 통일 — 단일 파란색 사용
        fig_bar = go.Figure(go.Bar(
        ...
        # AI 모델과 비교 — [요청 2] 일상 포함으로 바뀌었으므로 exclude_daily=True 명시
        if st.session_state.probs_history:
            avg_probs_for_cmp = get_avg_probs(exclude_daily=True, min_pct=0.0) or {}
            depression_emos = ["우울감", "슬픔", "무기력", "절망감", "죄책감"]
            model_depression_score = sum(avg_probs_for_cmp.get(e, 0) for e in depression_emos)
            st.markdown("---")
        ...
        st.markdown("---")
        # [요청 5] 이제 이 버튼은 최상위에서 렌더되므로 rerun 이후에도 정상적으로 페이지 이동됨
        col_go1, col_go2 = st.columns(2)
        with col_go1:
            if st.button("🏥 공공 서비스 연계 보기 →", type="primary",
                         key="btn_phq_to_service", use_container_width=True):
                st.session_state.page = "🏥 공공 서비스"
                st.rerun()
        with col_go2:
            if st.button("🌱 맞춤 케어 보기 →",
                         key="btn_phq_to_care", use_container_width=True):
                st.session_state.page = "🌱 맞춤 케어"
                st.rerun()
